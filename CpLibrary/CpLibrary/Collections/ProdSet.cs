...
namespace CpLibrary.Collections
{
	...
	public class ProdSet<T, TOp> : Set<T>, IEnumerable<T> where T : IComparable<T> where TOp : struct, IProdSetOperator<T>
	{
		T[] prod;
		private static readonly TOp op = default;

		public ProdSet(IComparer<T> comparer, bool isMultiSet = false) : base(comparer, isMultiSet)
		{
			prod = new T[1];
			prod[0] = op.Identity;
		}

		public ProdSet(IList<T> list, IComparer<T> comparer, bool isMultiSet = false) : base(list, comparer, isMultiSet)
		{
			prod = new T[list.Count + 1];
			prod[0] = op.Identity;
		}

		protected override int Build(IList<T> a, int l, int r)
		{
			if (l >= r) return 0;
			var m = (l + r) >> 1;
			var p = NewNode(a[m]);
			...
			prod[p] = n.Value;
			if (nodes[n.Left].Count > 0) prod[p] = op.Operate(prod[n.Left], prod[p]);
			if (nodes[n.Right].Count > 0) prod[p] = op.Operate(prod[p], prod[n.Right]);
			nodes[p] = n;
			Update(p);
			return p;
		}

		protected override void EnsureOneFreeSlot()
		{
			if (used + 1 < nodes.Length) return;
			int newLen = nodes.Length <= 1 ? 2 : nodes.Length << 1;
			Array.Resize(ref nodes, newLen);
			Array.Resize(ref prod, newLen);
		}

		protected override int NewNode(T value)
		{
			EnsureOneFreeSlot();
			int p = ++used;
		...
		protected override void Update(int p)
		{
			if (p == 0) return;
			var n = nodes[p];
		...
		private T Prod(int cur, int x, int a, int b, int l, int r)
		{
			if (nodes[cur].Count == 0) return op.Identity;
			if (r <= a || b <= l) return op.Identity;
			if (l <= a && b <= r) return prod[cur];
			var ret = l <= x && x < r ? nodes[cur].Value : op.Identity;
			if (nodes[nodes[cur].Left].Count > 0)
				ret = op.Operate(Prod(nodes[cur].Left, x - nodes[nodes[nodes[cur].Left].Right].Count - 1, a, x, l, r), ret);
			if (nodes[nodes[cur].Right].Count > 0)
				ret = op.Operate(ret, Prod(nodes[cur].Right, x + nodes[nodes[nodes[cur].Right].Left].Count + 1, x + 1, b, l, r));
			return ret;
		}
	}

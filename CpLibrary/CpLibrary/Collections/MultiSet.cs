...
namespace CpLibrary.Collections
{
	public class MultiSet<T> : Set<T> where T : IComparable<T>
	{
		...
		public MultiSet(IList<T> list) : base(list, true) { }
		public MultiSet(IList<T> list, IComparer<T> comparer) : base(comparer, true) { }
		public MultiSet(IList<T> list, Comparison<T> comparison) : base(list, comparison, true) { }
	}
}
